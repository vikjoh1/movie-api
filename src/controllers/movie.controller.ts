...
import { getMovieById, getMovieByTitle, searchMovies } from "../api/movie.api";
...
export const getMovieByImbdId = async (req: Request, res: Response) => {
...
export const setFavoriteMovie = async (req: Request, res: Response) => {
  ...
  try {
    const movieData = await getMovieByTitle(Title);
    const isMovieInFavorites = await movieRepo.isFavoriteMovie(
      userId,
      movieData.imdbID
    );
...
export const deleteFavoriteMovie = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const movieIndex = favoriteMovies.movies.findIndex(
